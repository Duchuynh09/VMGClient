...
export default function TableStage({ stages }: Props) {
    ...
    const renderCell = React.useCallback((stage: Stage, columnKey: React.Key) => {
        ...
        switch (columnKey) {
            ...
            default:
                return typeof cellValue === 'object' ? JSON.stringify(cellValue) : cellValue ?? ''; // ✅ fallback an toàn;
        }
    }, []);
    ...
    return (
        ...
        >
            <TableHeader columns={columns}>
                {(column) => (
                    <TableColumn key={column.accessKey} allowsSorting={column.sortable} align={column.accessKey === "actions" ? "center" : "start"}>
                        {column.name}
                    </TableColumn>
                )}
            </TableHeader>
